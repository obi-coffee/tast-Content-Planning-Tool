...
function ContentCard({ item, campaigns, onClick, compact, currentMember, commentCount = 0, selected, onToggleSelect }) {
  ...
  return (
    <>
      <div onClick={onClick} className="bg-white rounded-xl border cursor-pointer hover:border-brand-200 transition-all mb-2 overflow-hidden"
        ...
        <div className={compact?"p-2":"p-3"}>
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              <span className={`text-warm-text ${compact?"text-xs font-medium":"text-sm font-semibold"}`}>{item.title}</span>
              {!compact && item.product && <p className="text-xs text-warm-muted mt-0.5">{item.product}</p>}
            </div>
          ...
          {!compact && campaign?.keyMessage && <p className="text-xs text-warm-muted mt-1 line-clamp-1 italic font-serif">"{campaign.keyMessage}"</p>}
          {!compact && item.draftCopy && <p className="text-xs text-warm-muted mt-1 line-clamp-2 border-l-2 pl-2 border-warm-border">{item.draftCopy}</p>}
          <div className={`flex flex-wrap gap-1 ${compact?"mt-1":"mt-2"}`}>
            ...
            {channels.slice(0, compact?1:99).map(ch=><Tag key={ch} label={ch} colorClass="bg-cream-dark text-warm-muted" />)}
            {compact && channels.length>1 && <span className="text-xs text-warm-muted/50">+{channels.length-1}</span>}
          </div>
          {item.date && <p className="text-xs text-warm-muted/60 mt-1">{item.date}</p>}
          {!compact && (
            <div className="flex items-center justify-between mt-2">
              {item.assigneeId && <Avatar memberId={item.assigneeId} size={18} />}
              <button
                onClick={e=>{e.stopPropagation();setShowComments(true);}}
                className="text-xs text-warm-muted/50 hover:text-brand-300 transition-colors ml-auto flex items-center gap-1"
                title="Comments"
              >
                Comments
              </button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <h2 className="font-serif text-xl text-warm-text" style={{ fontWeight: 400 }}>Content Pipeline</h2>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => exportItemsToCSV(filteredItems)}
            className="text-xs px-3 py-1.5 rounded-lg border border-warm-border text-warm-muted hover:text-warm-text font-medium">
            Export CSV
          </button>
          ...
          {PIPELINE_STAGES.map(stage=>{
            ...
            return (
              <div key={stage}
                ...
                style={{width:isMobile?160:230,background:isOver?"#F0588110":"#f5f0e8",border:isOver?"2px dashed #F05881":"2px solid transparent"}}>
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-serif text-xl text-warm-text">Calendar</h2>
...
export function BrandVoice({ voice, setVoice }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-serif text-xl text-warm-text">Brand Voice & Guidelines</h2>
        {!editing
...
export function Captions({ brandVoice }) {
  ...
  return (
    <div>
      <h2 className="font-serif text-xl text-warm-text mb-1">Caption Generator</h2>
